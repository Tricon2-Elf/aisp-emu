...
public class AreaRoboCreateHandlerTests
{
    ...
    {
        var chara = new CharaData(RoboInventoryStore.ObjectIdFor(1), 1002011, "Robot");
        var bytes = new RoboData(1, chara).ToBytes();
        Assert.Equal(961, bytes.Length);
        Assert.Equal(965, new RoboCreateResponse(0, new RoboData(1, chara)).ToBytes().Length);
    }
    ...
    public async Task HandleAsync_SendsSuccessWithRoboData()
    {
        ...
        var handler = new AreaRoboCreateHandler(store, NullLogger<AreaRoboCreateHandler>.Instance);
        ...
        Assert.Equal(965, sent.Payload.Length);
