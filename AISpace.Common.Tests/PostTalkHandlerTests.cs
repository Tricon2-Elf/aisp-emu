...
public class PostTalkHandlerTests
{
    ...
    public async Task ForwardNotify_IncludesSenderCharacterId()
    {
        ...
        var recipient = new CapturingPlayerSession { User = CreateUser(2, 9002), UserId = 2, CharacterId = 9002 };
        ...
        await handler.HandleAsync(BuildPostTalkPayload(1, -1, "hello", 0), sender, TestContext.Current.CancellationToken);
    ...
    private static User CreateUser(int userId, int characterId)
    {
        ...
        user.Characters.Add(new Character { Id = characterId, Name = $"char{characterId}", UserId = userId });
        return user;
    }

    private static byte[] BuildPostTalkPayload(uint messageId, int distId, string message, uint balloonId)
    {
