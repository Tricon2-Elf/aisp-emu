...
public sealed class AreaPresenceIndex(ISessionClientRegistry sessionClientRegistry)
    : IAreaPresenceIndex
{
    public IReadOnlyList<IPlayerSession> GetAreaSessions(uint mapId, int channelId)
    {
        return sessionClientRegistry
            .GetClients(ServerType.Area)
            .Where(session => IsInArea(session, mapId, channelId))
            .ToList();
    }
    ...
    )
    {
        ...
        return includeSelf
            ? peers
            : peers.Where(other => other.ConnectionId != session.ConnectionId).ToList();
    }
    ...
    {
        IEnumerable<IPlayerSession> candidates = sessionClientRegistry
            .GetClients(ServerType.Area)
            .Where(session => session.CharacterId == characterId);

        if (mapId.HasValue)
            candidates = candidates.Where(session =>
                IsInArea(session, mapId.Value, channelId ?? 0)
            );
    ...
    )
    {
        IEnumerable<IPlayerSession> candidates = sessionClientRegistry
            .GetClients(ServerType.Area)
            .Where(session => (session.User?.Id ?? session.UserId) == userId);
