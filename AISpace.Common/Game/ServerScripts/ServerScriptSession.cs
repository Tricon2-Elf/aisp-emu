...
{
    public void Begin(IPlayerSession session, string eventKey, EventCompletionPolicy completionPolicy)
    {
    ...
    public async Task CompleteAsync(IPlayerSession session, uint result, bool markComplete, CancellationToken ct = default)
    {
        ...
        if (markComplete && completionPolicy == EventCompletionPolicy.Once && session.CharacterId != 0 && eventKey is not null)
        {
