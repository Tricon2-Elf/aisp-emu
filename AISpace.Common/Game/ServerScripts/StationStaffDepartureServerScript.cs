...
public sealed class StationStaffDepartureServerScript(ICharacterRepository characterRepository, ClientScriptSegmentRunner clientScriptSegmentRunner, ServerScriptSession serverScriptSession, DirectMapLinkTransitionService directMapLinkTransitionService, ILogger<StationStaffDepartureServerScript> logger) : IServerScript
{
    ...
    private static readonly IReadOnlyDictionary<uint, DepartureRoute> Routes = new Dictionary<uint, DepartureRoute>
    {
    ...
    public async Task StartAsync(IPlayerSession session, ServerScriptContext context, CancellationToken ct = default)
    {
        ...
        {
            logger.LogWarning("Aborting server script {EventKey} for character {CharacterId}: character not found", EventKey, session.CharacterId);
        ...
        if (character.HomeIslandId == 0)
        {
            ...
            await session.SendAsync(PacketType.EventMessageNotify, new EventMessageNotify(npcObjectId, context.Npc.Name, "Please register at the Sotokanda Building first.").ToBytes(), ct);
        ...
        {
            logger.LogWarning("Aborting server script {EventKey} for character {CharacterId}: unsupported home island {IslandId}", EventKey, session.CharacterId, character.HomeIslandId);
            await serverScriptSession.AbortAsync(session, EventFailure, ct);
            return;
        }

        session.ServerScriptState!.Data[DestinationMapDataKey] = route.DestinationMapId;
        logger.LogInformation("Starting client script segment {ClientScriptKey} for character {CharacterId} before travel to map {DestinationMapId}", route.ClientScriptKey, session.CharacterId, route.DestinationMapId);
    ...
    public async Task<bool> TryHandlePacketAsync(PacketType packetType, ReadOnlyMemory<byte> payload, IPlayerSession session, CancellationToken ct = default)
    {
        ...
        if (state.Step == AwaitingRegistrationMessageSyncStep && packetType == PacketType.EventSyncRRequest)
        {
        ...
        switch (segmentResult.Status)
        {
            ...
            case ClientScriptSegmentStatus.Completed:
                ...
                if (!await directMapLinkTransitionService.TryTeleportToMapAsync(session, destinationMapId, ct))
                    logger.LogWarning("Server script {EventKey} completed for character {CharacterId}, but teleport to map {DestinationMapId} failed", EventKey, session.CharacterId, destinationMapId);
                return true;
            default:
