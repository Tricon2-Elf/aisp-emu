...
public sealed class StationStaffReturnToAkihabaraServerScript(ServerScriptSession serverScriptSession, DirectMapLinkTransitionService directMapLinkTransitionService, ILogger<StationStaffReturnToAkihabaraServerScript> logger) : IServerScript
{
    ...
    public async Task StartAsync(IPlayerSession session, ServerScriptContext context, CancellationToken ct = default)
    {
        ...
        await session.SendAsync(PacketType.EventMessageNotify, new EventMessageNotify(npcObjectId, context.Npc.Name, "I'll take you to Akihabara").ToBytes(), ct);
    ...
    public async Task<bool> TryHandlePacketAsync(PacketType packetType, ReadOnlyMemory<byte> payload, IPlayerSession session, CancellationToken ct = default)
    {
        ...
        if (state is null || !string.Equals(state.EventKey, EventKey, StringComparison.Ordinal) || state.Step != AwaitingMessageSyncStep || packetType != PacketType.EventSyncRRequest)
            return false;
        ...
        if (!await directMapLinkTransitionService.TryTeleportToMapAsync(session, AkihabaraMapId, ct))
            logger.LogWarning("Server script {EventKey} completed for character {CharacterId}, but teleport to map {DestinationMapId} failed", EventKey, session.CharacterId, AkihabaraMapId);
