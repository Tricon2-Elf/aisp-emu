...
) : IPacketHandler, IRequiresAuthenticatedSession
{
    ...
    )
    {
        ...
        logger.LogInformation(
            "MapEnterRequest from user {UserId}: requested MapID {MapId}, ChannelID {ChannelId}",
        ...
        if (session.IsMapTransitionPending)
        {
            ...
            {
                logger.LogWarning(
                    "Rejecting MapEnterRequest for user {UserId} during pending transition: requested map {RequestedMapId}, channel {RequestedChannelId}, but session is on map {SessionMapId}, channel {SessionChannelId}",
                    session.User?.Id ?? session.UserId,
                    request.MapID,
                    request.ChannelId,
                    session.MapId,
            ...
            logger.LogInformation(
                "Acknowledging post-NotifyChangeMap MapEnterRequest for user {UserId} on map {MapId}, channel {ChannelId}",
                session.User?.Id ?? session.UserId,
                request.MapID,
                request.ChannelId
            );
        ...
        {
            logger.LogWarning(
                "Rejecting MapEnterRequest for user {UserId}: character could not be resolved",
                session.User?.Id ?? session.UserId
            );
            await session.SendAsync(ResponseType, new AreaMapEnterResponse(1).ToBytes(), ct);
            return;
        ...
        if (request.MapID == session.MapId && request.ChannelId == (uint)session.ChannelId)
        {
            ...
            {
                logger.LogInformation(
                    "Treating MapEnterRequest for current map {MapId} and channel {ChannelId} as a post-load acknowledgement for user {UserId}; no movement has been observed on this map yet",
                    request.MapID,
                    request.ChannelId,
                    session.User?.Id ?? session.UserId
                );
            ...
            if (
                await directMapLinkTransitionService.TryHandleMapEnterTriggerAsync(
                    request,
                    session,
                    ct
                )
            ...
            logger.LogInformation(
                "No direct MapLink matched current-map MapEnterRequest for user {UserId} on map {MapId}, channel {ChannelId}, position ({X}, {Y}, {Z}); treating as no-op acknowledgement",
                session.User?.Id ?? session.UserId,
                request.MapID,
                request.ChannelId,
                session.X,
        ...
        await directMapLinkTransitionService.CompleteMapTransitionAsync(
            session,
            character,
            request.MapID,
            request.ChannelId,
            map,
            notifyChangeMap: null,
    ...
    private async Task TryNotifyServerScriptsAsync(
        ReadOnlyMemory<byte> payload,
        IPlayerSession session,
