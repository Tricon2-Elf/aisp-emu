...
public class AreaOtherProfileTextHandler(SharedState state) : IPacketHandler, IRequiresAuthenticatedSession
{
    ...
    public async Task HandleAsync(ReadOnlyMemory<byte> payload, IPlayerSession session, CancellationToken ct = default)
    {
        ...
        var target = state.GetAreaSessionByCharacterId(req.TargetObjectId, session.MapId, session.ChannelId);
        var cha = target?.Character ?? target?.User?.Characters.FirstOrDefault();

        var profile = cha != null ? new ProfileData(cha.Like1 ?? "None", cha.Like2 ?? "None", cha.Like3 ?? "None", cha.LikeDesc1 ?? "", cha.LikeDesc2 ?? "", cha.LikeDesc3 ?? "", cha.AvatarDesc ?? "Hello!") : new ProfileData("", "", "", "", "", "", "");
