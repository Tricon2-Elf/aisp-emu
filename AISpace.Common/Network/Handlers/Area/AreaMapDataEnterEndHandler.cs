...
public class AreaMapDataEnterEndHandler(ILogger<AreaMapDataEnterEndHandler> logger, SharedState state) : IPacketHandler
{
    ...
    public async Task HandleAsync(ReadOnlyMemory<byte> payload, ClientConnection connection, CancellationToken ct = default)
    {
        ...
        // Данные о моем персонаже для рассылки другим
        ...
        foreach (var other in state.AreaClients.Values)
        {
            ...
            // Сообщаем другому игроку, что я зашел
            await other.SendAsync(PacketType.AvatarNotifyData, spawnMePacket, ct);

            // Получаем данные другого игрока, чтобы заспавнить его у СЕБЯ на экране
        ...
        // Цикл с SpawnFakePlayer удален, ботов больше не будет.
    }
