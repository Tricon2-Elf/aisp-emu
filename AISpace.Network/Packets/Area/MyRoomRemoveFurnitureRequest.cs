...
public sealed class MyRoomRemoveFurnitureRequest(uint roomId, uint furnitureId)
    : IIncomingPacket<MyRoomRemoveFurnitureRequest>
{
    ...
    public static MyRoomRemoveFurnitureRequest FromBytes(ReadOnlySpan<byte> data)
    {
        if (data.Length != WireSize)
            throw new InvalidDataException(
                $"{nameof(MyRoomRemoveFurnitureRequest)} requires exactly {WireSize} bytes, received {data.Length}."
            );
