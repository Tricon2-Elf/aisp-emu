...
public sealed class MyRoomSetFurnitureRequest(uint roomId, uint serialId, MyRoomFurnitureTransform transform) : IIncomingPacket<MyRoomSetFurnitureRequest>
{
    ...
    public static MyRoomSetFurnitureRequest FromBytes(ReadOnlySpan<byte> data)
    {
        if (data.Length != WireSize)
            throw new InvalidDataException($"{nameof(MyRoomSetFurnitureRequest)} requires exactly {WireSize} bytes, received {data.Length}.");

        var reader = new PacketReader(data);
        var roomId = reader.ReadUInt();
        var serialId = reader.ReadUInt();
        return new MyRoomSetFurnitureRequest(roomId, serialId, MyRoomFurnitureTransform.Read(ref reader));
    }
}
