...
public sealed class MyRoomUpdateFurnitureRequest(uint roomId, uint furnitureId, MyRoomFurnitureTransform transform) : IIncomingPacket<MyRoomUpdateFurnitureRequest>
{
    ...
    public static MyRoomUpdateFurnitureRequest FromBytes(ReadOnlySpan<byte> data)
    {
        if (data.Length != WireSize)
            throw new InvalidDataException($"{nameof(MyRoomUpdateFurnitureRequest)} requires exactly {WireSize} bytes, received {data.Length}.");

        var reader = new PacketReader(data);
        var roomId = reader.ReadUInt();
        var furnitureId = reader.ReadUInt();
        return new MyRoomUpdateFurnitureRequest(roomId, furnitureId, MyRoomFurnitureTransform.Read(ref reader));
    }
}
