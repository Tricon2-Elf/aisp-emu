...
public sealed class MyRoomUpdateNameRequest(uint roomId, string name) : IIncomingPacket<MyRoomUpdateNameRequest>
{
    ...
    public static MyRoomUpdateNameRequest FromBytes(ReadOnlySpan<byte> data)
    {
        if (data.Length < sizeof(uint) + 1 || data.Length > MaximumPayloadSize)
            throw new InvalidDataException($"{nameof(MyRoomUpdateNameRequest)} requires a 4-byte room ID and a null-terminated name within a 46-byte field.");
        if (data[^1] != 0)
            throw new InvalidDataException($"{nameof(MyRoomUpdateNameRequest)} name is not null terminated.");
