    ...
    where T : GameServerBase<T>
{
    ...
    protected GameServerBase(ILogger<T> logger, GameServerContext ctx, int port, string healthKey)
    {
        Logger = logger;
        Db = ctx.Db;
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        ...
        var listener = new VceListener(_loggerFactory.CreateLogger<VceListener>(), _channel, ActiveServerType.ToString(), _port, _loggerFactory, id => State.UnregisterClient(ActiveServerType, id), (_, p) => HealthRegistry.MarkListening(_healthKey, p), _maxConcurrentClients);
