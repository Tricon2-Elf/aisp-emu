...
public class ScheduledMaintenanceService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        if (!_options.Enabled)
        {
            _logger.LogInformation("Scheduled maintenance is disabled");
            return;
        }
        ...
        if (!parsed)
        {
            _logger.LogError("Invalid ScheduledTime format '{ScheduledTime}'. Expected 'HH:mm' (UTC). Maintenance disabled.", _options.ScheduledTime);
            return;
        }

        _logger.LogInformation("Scheduled maintenance enabled: daily at {ScheduledTime} UTC with warnings at {Minutes} min", _options.ScheduledTime, string.Join(", ", _options.WarningMinutes));
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            if (timeUntil.TotalSeconds <= 0 && !_shutdownTriggered)
            {
                ...
                BroadcastShutdown();
                _logger.LogInformation("Scheduled shutdown triggered. Stopping application.");
    ...
    private void BroadcastWarning(int minutesRemaining)
    {
        ...
        _logger.LogInformation("Broadcasting maintenance warning: {Minutes} min remaining", minutesRemaining);
    ...
    private void BroadcastShutdown()
    {
        ...
        _logger.LogInformation("Broadcasting shutdown notice");
        SendToAllAreaClients(message);
    }
