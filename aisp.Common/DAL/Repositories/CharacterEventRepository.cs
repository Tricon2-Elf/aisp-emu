...
public interface ICharacterEventRepository
{
    ...
    Task<IReadOnlySet<string>> GetCompletedEventKeysAsync(
        int characterId,
...
public sealed class CharacterEventRepository(MainContext db) : ICharacterEventRepository
{
    public async Task<bool> HasCompletedAsync(
        ...
        await db.CharacterEventStatuses.AnyAsync(
            x => x.CharacterId == characterId && x.EventKey == eventKey,
            ct
        );

    public async Task<IReadOnlySet<string>> GetCompletedEventKeysAsync(
        int characterId,
        CancellationToken ct = default
    )
    {
        var keys = await db
            .CharacterEventStatuses.Where(x => x.CharacterId == characterId)
            .Select(x => x.EventKey)
            .ToListAsync(ct);
