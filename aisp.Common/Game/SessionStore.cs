...
public sealed class SessionStore : ISessionStore
{
    ...
    public bool TryGetSession(Guid connectionId, out IPlayerSession? session) =>
        _sessionByConnectionId.TryGetValue(connectionId, out session);

    public IReadOnlyList<IPlayerSession> GetSessions() => _sessionByConnectionId.Values.ToList();

    public bool RemoveSession(Guid connectionId) =>
        _sessionByConnectionId.TryRemove(connectionId, out _);
}
