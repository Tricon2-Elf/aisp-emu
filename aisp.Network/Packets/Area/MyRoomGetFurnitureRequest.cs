...
/// </summary>
public sealed class MyRoomGetFurnitureRequest(uint mapId, uint channelId)
    : IIncomingPacket<MyRoomGetFurnitureRequest>
{
    ...
    public static MyRoomGetFurnitureRequest FromBytes(ReadOnlySpan<byte> data)
    {
        if (data.Length != WireSize)
            throw new InvalidDataException(
                $"{nameof(MyRoomGetFurnitureRequest)} requires exactly {WireSize} bytes, received {data.Length}."
            );
